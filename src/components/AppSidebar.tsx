...
const navItems = [
  ...
  { to: '/pulse', label: 'Pulse', icon: Bell, usePulseCount: true },
];
    ...
    <aside className="hidden lg:flex flex-col w-[220px] min-h-screen bg-card border-r fixed left-0 top-0 z-30">
      ...
      <nav className="flex-1 px-3 space-y-0.5">
        {navItems.map(item => {
          ...
          return (
            <NavLink key={item.to} to={item.to} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-150 ${isActive ? 'bg-primary-light text-sidebar-accent-foreground border-l-2 border-primary font-medium' : 'text-muted-foreground hover:bg-muted'}`}>
              <item.icon size={18} />
      ...
      <div className="p-3 border-t">
        <NavLink to="/settings" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${location.pathname === '/settings' ? 'bg-primary-light text-sidebar-accent-foreground' : 'text-muted-foreground hover:bg-muted'}`}>
    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-card border-t z-30 px-2 pb-[env(safe-area-inset-bottom)]">
      <div className="flex justify-around">
        {mobileNavItems.map(item => {
          const isActive = location.pathname === item.to || location.pathname.startsWith(item.to + '/');
          return (
            <NavLink key={item.to} to={item.to} className={`flex flex-col items-center py-2.5 px-3 text-[11px] transition-colors relative ${isActive ? 'text-primary' : 'text-muted-foreground'}`}>
              <item.icon size={20} />
