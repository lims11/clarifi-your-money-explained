...
import { CreditCard, HandCoins, Car, Shield, Wallet, Home, ArrowRight, Star, BadgePercent, TrendingUp } from 'lucide-react';
...
const creditCardOffers = [
  {
    ...
    providerLetter: 'A',
    providerColor: '#007BC1',
    tag: 'Clarifi Pick',
    features: [
      { label: '5% cashback', desc: 'For the first 3 months' },
      { label: '1% ongoing', desc: 'On all purchases' },
      { label: '94%', desc: 'Approval likelihood' },
    ],
    highlight: 'Based on your score of 724, you have a high chance of approval',
  },
  {
    id: 2,
    name: 'Loqbox Credit Builder',
    provider: 'Loqbox',
    providerLetter: 'L',
    providerColor: '#2D2B55',
    tag: null,
    features: [
      { label: 'Free or £2.99', desc: 'Weekly membership' },
      { label: 'Improve your score', desc: 'And reach your savings goal' },
      { label: '100% approval', desc: 'With no hard credit checks' },
    ],
    highlight: null,
  },
  {
    id: 3,
    name: 'Chase Cashback Card',
    provider: 'Chase UK',
    providerLetter: 'C',
    providerColor: '#117ACA',
    tag: 'Exclusive',
    features: [
      { label: '1% cashback', desc: 'On all spending' },
      { label: 'No annual fee', desc: 'Free forever' },
      { label: '89%', desc: 'Approval likelihood' },
    ],
    highlight: null,
  },
];
...
const loanOffers = [
  {
    id: 4,
    name: 'Personal loans from Aspire Money',
    provider: 'Aspire Money',
    providerLetter: 'A',
    providerColor: '#1D9E75',
    features: [
      { label: '1 – 5 years', desc: 'Loan duration' },
      { label: '6.9% APR', desc: 'Representative APR' },
      { label: '£500 – £25,000', desc: 'Loan amount' },
    ],
  },
  {
    id: 5,
    ...
    provider: 'Zopa',
    providerLetter: 'Z',
    providerColor: '#00B4A0',
    features: [
      { label: '1 – 7 years', desc: 'Loan duration' },
      { label: '5.9% APR', desc: 'Representative APR' },
      { label: '£1,000 – £50,000', desc: 'Loan amount' },
    ],
  },
];
...
const carFinanceOffers = [
  {
    ...
    icon: '💷',
    desc: 'Pay monthly and own the car at the end',
  },
  {
    id: 7,
    name: 'Personal Contract Purchase (PCP)',
    icon: '🚗',
    desc: 'Lower monthly payments, with flexibility at the end',
  },
  {
    id: 8,
    name: 'Switch your car loan',
    icon: '🔄',
    desc: "See if you're over paying and switch to a new loan",
  },
  {
    id: 9,
    name: 'Finance your final PCP payment',
    icon: '📋',
    desc: 'Spread the cost of your balloon payment',
  },
];
    ...
    <div className="p-5 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl p-6 text-primary-foreground relative overflow-hidden" style={{ background: 'linear-gradient(135deg, hsl(var(--primary)), hsl(220 25% 18%))' }}>
        <div className="relative z-10">
          ...
          <button className="text-sm font-medium mt-3 text-teal hover:underline flex items-center gap-1">
            Browse loan offers <ArrowRight size={14} />
          </button>
        </div>
        <div className="absolute top-0 right-0 w-40 h-full opacity-10">
          <svg viewBox="0 0 160 120" className="w-full h-full">
            <circle cx="120" cy="30" r="60" fill="currentColor" />
            <circle cx="140" cy="90" r="40" fill="currentColor" />
          </svg>
        </div>
      </div>
        ...
        {offerCategories.map(cat => {
          ...
          return (
            <button
              ...
              className={`clarifi-card text-left p-4 transition-all hover:shadow-md ${isActive ? 'ring-2 ring-primary' : ''}`}
            >
      ...
      {activeCategory === 'credit-cards' && (
        <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground -mt-2">These cards are matched to your credit score and spending patterns.</p>
          ...
          <div className="space-y-4">
            {creditCardOffers.map(offer => (
              <div key={offer.id} className="clarifi-card p-5">
                ...
                {offer.highlight && (
                  <p className="text-xs text-teal mt-3 flex items-center gap-1">
                    <TrendingUp size={12} /> {offer.highlight}
                  </p>
                )}
      ...
      {activeCategory === 'loans' && (
        <div className="space-y-4">
          <h2 className="text-base font-semibold">Unsecured Loans</h2>
          <p className="text-sm text-muted-foreground -mt-2">Personal loans based on your credit score and history.</p>
      ...
      {activeCategory === 'car-finance' && (
        <div className="space-y-5">
          <div>
            <h2 className="text-base font-semibold">Looking for your next car?</h2>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {carFinanceOffers.slice(0, 2).map(offer => (
              <div key={offer.id} className="clarifi-card p-5 hover:shadow-md transition-shadow cursor-pointer">
                <span className="text-2xl mb-3 block">{offer.icon}</span>
                <h3 className="text-sm font-semibold">{offer.name}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {carFinanceOffers.slice(2).map(offer => (
              <div key={offer.id} className="clarifi-card p-5 hover:shadow-md transition-shadow cursor-pointer">
                <span className="text-2xl mb-3 block">{offer.icon}</span>
                <h3 className="text-sm font-semibold">{offer.name}</h3>
                <p className="text-xs text-muted-foreground mt-1">{offer.desc}</p>
      ...
      {activeCategory === 'insurance' && (
        <div className="space-y-4">
          <h2 className="text-base font-semibold">Car Insurance</h2>
          <div className="clarifi-card p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="text-sm font-semibold">Compare car insurance quotes</h3>
                <p className="text-xs text-muted-foreground">Get a quote in minutes. No impact on your credit score.</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3 py-3 border-y">
      ...
      {activeCategory === 'accounts' && (
        <div className="space-y-4">
          <h2 className="text-base font-semibold">Current Accounts</h2>
          <p className="text-sm text-muted-foreground -mt-2">Switch or open a new current account.</p>
          <div className="clarifi-card p-5">
            <div className="flex items-start justify-between mb-3">
              <div>
                <span className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-md bg-primary/10 text-primary mb-2">
                  <BadgePercent size={10} /> Switching bonus
                </span>
      ...
      {activeCategory === 'mortgages' && (
        <div className="space-y-4">
          <h2 className="text-base font-semibold">Mortgages</h2>
          <p className="text-sm text-muted-foreground -mt-2">Buy a home or get remortgaging advice.</p>
          <div className="clarifi-card p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="text-sm font-semibold">Get mortgage advice</h3>
                <p className="text-xs text-muted-foreground">Talk to a qualified mortgage broker for free</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3 py-3 border-y">
